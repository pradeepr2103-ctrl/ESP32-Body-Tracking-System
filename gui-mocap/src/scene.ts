  ...
  private applyMixamoPose(bones: BoneT[]) {
    ...
    for (const bp of order) {
      ...
      if (!mixamoBone.parent || !(mixamoBone.parent instanceof THREE.Bone)) {
        // Root bone (HIP): rotationG IS world space, and with no parent,
        // world space == local space here. No conversion needed.
        mixamoBone.quaternion.copy(this.worldQuat);
      } else {
        // Non-root bone: rotationG is world-space, but THREE bones store
        // LOCAL rotation. Must convert: local = inverse(parentWorld) * world.
        // Critical fix: parent's world matrix must reflect THIS frame's
        // already-applied rotation, not last frame's stale transform — so
        // we force a matrix-world update after every bone in the chain.
        // Without this, every bone after the first reads its parent's
        // PREVIOUS frame rotation, producing a 90°-style snap on small
        // movements because the conversion is against a stale reference.
        mixamoBone.parent.updateWorldMatrix(true, false);
      ...
      // Push this bone's new local rotation into the world matrix chain
      // immediately, so the NEXT bone in `order` (its child) computes its
      // parent-relative conversion against up-to-date data.
